...
class CSV_TO_STRUCT:
  ...
  def gen_code(self, page):
    rsv_cnt = 0
    template = "  bit [msb:lsb] fields; //comments\n"
    sub_tbl = ""
    prefix = ""
    if "sub_tbl" in self.csv_fname:
      prefix = "//"
      sub_tbl = "_SUB_TBL" + self.csv_fname.split("sub_tbl")[-1].split(".csv")[0]
      print("csv_fname {} sub_tbl {}".format(self.csv_fname, sub_tbl))
    if "___" in self.csv_fname:
      s_name = "S_" + self.csv_fname.split("_")[0].upper() + sub_tbl
    else:
      s_name = "S_" + self.csv_fname.split(".csv")[0].upper() + sub_tbl
    if page in self.code_dict.keys():
      s = self.code_dict[page] + prefix + "typedef struct packed {\n"
    else:
      s = prefix + "typedef struct packed {\n"
    for csv_info in self.csv_info_lst:
      ...
      if "(" in csv_info["Description"]:
        ...
        comments = ""
        rsv_cnt += 1
      s += prefix + template.replace("msb", msb).replace("lsb", lsb).replace("fields", fields).replace("comments", comments)
    s += prefix + "} " + s_name + "; //nvme spec page {}".format(page) + "\n\n\n"
    self.code_dict[page] = s

  def write_struct(self):
    fw = open(self.struct_lib, self.write_mode)
    fw.writelines("//This file is generated by csv_to_struct.py\n")
    keys = []
    for key in self.code_dict.keys():
      keys.append(key)
    keys.sort()
    for key in keys:
      s = self.code_dict[key]
      fw.writelines(s)
    fw.close()
  ...
  def get_csv_info(self):
    ...
    for line in fr.readlines():
      if "extracted from page" in line:
        page = line.split("extracted from page")[-1].strip(" \n")
